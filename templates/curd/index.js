module.exports = {
    description: "create norm of curd model",
    prompts: [
        {
            type: "input",
            message: "请输入模型名称(用-分隔单词, 统一小写),勿与之前重复,然后点击回车",
            name: "modelName",
          },
          {
            type: "input",
    ...
    actions: (data) => {
        ...
        const listPath = isPackage? `${modelName}` : `${packageName}/${modelName}`
        const editDialogPath = isPackage? `${modelName}/edit-dialog.vue` : `${packageName}/${modelName}-edit-dialog.vue`
        console.log(data)
        return [
            {
                type: "add",
